...
The seventeenth pilot dataset tests Lapita expansion, Polynesian navigation, ceremonial landscapes, island settlement routes, and Oceania exchange networks:
...
The nineteenth pilot dataset tests Indian Ocean entrepots, Red Sea and Persian Gulf gateways, Malabar spice ports, the Straits of Malacca, Song-Yuan maritime trade, and long-distance spice, horse, textile, pearl, and ceramic exchange:
...
The twenty-second pilot dataset tests early modern East Asian maritime trade ports, restricted trade systems, the Canton system, treaty ports, and Korean/Japanese open-port urbanism:
...
## Seventeen-Dataset Public Usability Pass

The 2026-05-22 seventeen-dataset browser pass checks `/stage2-preview.html` after adding the Lapita and Pacific settlement landscapes pilot dataset:

- Dataset selector exposes 17 datasets.
- All 17 indexed datasets render their expected entry, marker, and permanent-label counts: Atlantic revolutions 8, Ethiopia 8, Mesopotamia 10, Egypt 14, Indus 8, Early China 8, Ancient Greece 10, Achaemenid Persia 8, Roman Republic and Empire 8, Magadha-Maurya 8, Mesoamerica 8, West Africa 8, Andes 8, Southeast Asia 8, Maya lowland/Postclassic 8, Swahili Coast 8, and Lapita/Pacific 8.
- Compact context filters stay limited to 4 visible context buttons plus an expansion control after every dataset switch.
- Searching `라파누이` in the Lapita/Pacific dataset narrows the list and map to 1 entry, marker, and label.
- Selecting `라파누이` opens the detail panel with source-confidence information and detail actions.
- Closing the `라파누이` detail panel preserves the current search marker and label.
- The `서폴리네시아 Lapita 정착` context filter returns 2 entries: `누쿨레카 Lapita` and `물리파누아 Lapita`.
...
## Nineteen-Dataset Public Usability Pass

The 2026-05-22 nineteen-dataset browser pass checks `/stage2-preview.html` after adding the Indian Ocean trade ports and entrepots pilot dataset:

- Dataset selector exposes 19 datasets.
- All 19 indexed datasets render their expected entry, marker, and permanent-label counts: Atlantic revolutions 8, Ethiopia 8, Mesopotamia 10, Egypt 14, Indus 8, Early China 8, Ancient Greece 10, Achaemenid Persia 8, Roman Republic and Empire 8, Magadha-Maurya 8, Mesoamerica 8, West Africa 8, Andes 8, Southeast Asia 8, Maya lowland/Postclassic 8, Swahili Coast 8, Lapita/Pacific 8, North American Indigenous mound/urban centers 8, and Indian Ocean trade ports 8.
- Compact context filters stay limited to 4 visible context buttons plus an expansion control after every dataset switch.
- Searching `말라카` in the Indian Ocean dataset narrows the list and map to 1 entry, marker, and label.
- Selecting `말라카` opens the detail panel with source-confidence information and detail actions.
- Closing the `말라카` detail panel preserves the current search marker and label.
- The `페르시아만 이슬람 중계항` context filter returns 3 entries: `호르무즈`, `바스라`, and `시라프`.
...
## Thirty-Third Pilot Dataset

The Japanese ancient, medieval, and early modern heritage pilot dataset adds 8 entries:
...
## Thirty-Three-Dataset Public Usability Pass

The 2026-05-22 thirty-three-dataset public browser pass checks `https://wgis.vercel.app/stage2-preview.html` after adding the Japanese ancient, medieval, and early modern heritage pilot dataset:

- Dataset selector exposes 33 datasets.
- All 33 indexed datasets render their expected entry, marker, and permanent-label counts across 275 Stage 2 preview entries.
- The new Japanese heritage dataset exposes 8 entries: `호류지`, `도다이지`, `엔랴쿠지`, `뵤도인 봉황당`, `기요미즈데라`, `이쓰쿠시마 신사`, `히메지성`, and `닛코 도쇼구`.
- Compact context filters stay limited to 4 visible context buttons plus an expansion control.
- Searching `히메지` in the Japanese heritage dataset narrows the list and map to `히메지성` with 1 marker and 1 label.
- Selecting `닛코 도쇼구` opens the detail panel with source-confidence information and detail actions.
- The `헤이안 교토 불교와 정토 경관` context filter returns `엔랴쿠지`, `뵤도인 봉황당`, and `기요미즈데라` with 3 markers and 3 labels and no stale castle data.
- On mobile, `이쓰쿠시마 신사` detail renders with source-confidence information and both detail actions.
- Browser screenshots are saved to `C:\Users\Public\Documents\ESTsoft\CreatorTemp\wgis-stage2-thirty-three-datasets-desktop.png` and `C:\Users\Public\Documents\ESTsoft\CreatorTemp\wgis-stage2-thirty-three-datasets-mobile.png` when the local headless browser runtime is available; otherwise the script reports the skipped browser probe reason after completing dataset endpoint checks.

## Thirty-Fourth Pilot Dataset

The Chinese imperial ritual, defense, and Buddhist heritage pilot dataset adds 8 entries:
...
## Thirty-Four-Dataset Public Usability Pass

The 2026-05-22 thirty-four-dataset public browser pass checks `https://wgis.vercel.app/stage2-preview.html` after adding the Chinese imperial ritual, defense, and Buddhist heritage pilot dataset:

- Dataset selector exposes 34 datasets.
- All 34 indexed datasets render their expected entry, marker, and permanent-label counts across 283 Stage 2 preview entries.
- The new Chinese imperial heritage dataset exposes 8 entries: `자금성`, `천단`, `이화원`, `청더 피서산장`, `명13릉`, `만리장성 바다링 구간`, `윈강석굴`, and `다쭈석각`.
- Compact context filters stay limited to 4 visible context buttons plus an expansion control.
- Searching `천단` in the Chinese imperial heritage dataset narrows the list and map to `천단` with 1 marker and 1 label.
- Selecting `자금성` opens the detail panel with source-confidence information and detail actions.
