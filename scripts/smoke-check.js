...
function main() {
  ...
  check('first-stage entry floor', entries.length >= 275, `${entries.length} entries found`);
  check('curation batch floor', Array.isArray(batches.batches) && batches.batches.length >= 34);
  ...
  check('stage 2 index has multiple datasets', stage2Datasets.length >= 33);
